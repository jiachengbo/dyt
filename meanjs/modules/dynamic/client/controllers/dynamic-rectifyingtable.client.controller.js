(function () {
  ...
  angular
    .module('organization')
    .controller('RectifyingTableController', RectifyingTableController);

  RectifyingTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'RectifyingServive',
    '$uibModal', 'Authentication', 'CommunityService', 'Upload'];
  function RectifyingTableController($scope, Notification, $log, $window, RectifyingServive, $uibModal, Authentication, CommunityService, Upload) {
    var vm = this;
    vm.userCommId = '';
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/dynamic/client/views/rectifying-modal-form.client.view.html',
        controller: 'RectifyinginfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        RectifyingData: function () {
          //空数据
          return new RectifyingServive();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/rectifyinginfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党组织整顿新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党组织整顿删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/rectifyinginfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 党组织整顿修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      data: vm.tableData,
      columnDefs: [
        {field: 'name', displayName: '党组织名称', width: 150},
        {field: 'peoplenum', displayName: '党员人数', width: 80},
        ...
        {field: 'mainproblem', displayName: '存在的主要问题', width: 400},
        {field: 'Measures', displayName: '整改措施', width: 400}
      ],
    ...
    function refreshRecordCount(queryParam) {
      RectifyingServive.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return RectifyingServive.query(pageParam).$promise
        .then(function (data) {
