(function () {
  ...
  angular
    .module('organization')
    .controller('LearningDynamicsTableController', LearningDynamicsTableController);

  LearningDynamicsTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'LearningDynamicsServive', '$uibModal', 'Authentication', 'CommunityService', 'localStorageService'];
  function LearningDynamicsTableController($scope, Notification, $log, $window,
                                           Upload, LearningDynamicsServive, $uibModal, Authentication, CommunityService, localStorageService) {
    var vm = this;
    vm.userCommId = '';
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/dynamic/client/views/learningdynamics-modal-form.client.view.html',
        controller: 'LearningDynamicsinfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        LearningDynamicsData: function () {
          //空数据
          return new LearningDynamicsServive();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/learningdynamicsinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 学习动态新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 学习动态删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/learningdynamicsinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 学习动态修改成功!'});
            })
            .catch(function (err) {
    ...
    function refreshRecordCount(queryParam) {
      LearningDynamicsServive.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return LearningDynamicsServive.query(pageParam).$promise
        .then(function (data) {
