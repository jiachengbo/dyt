(function () {
  ...
  angular
    .module('organization')
    .controller('julacTableController', julacTableController);

  julacTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'julacServive', '$uibModal', 'Upload', 'Authentication', 'CommunityService'];
  function julacTableController($scope, Notification, $log, $window, julacServive, $uibModal, Upload, Authentication, CommunityService) {
    var vm = this;
    vm.userCommId = '';
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/dynamic/client/views/julac-modal-form.client.view.html',
        controller: 'julacinfovmdalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          ...
          .then(function (res) {
            ...
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 街道党建联席会新增成功!'});
          })
          .catch(function (err) {
      ...
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          Notification.success({
            message: '<i class="glyphicon glyphicon-ok"></i> 街道党建联席会删除成功!'
          });
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/julacinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 街道党建联席会修改成功!'});
            })
            .catch(function (err) {
    ...
    function refreshRecordCount(queryParam) {
      julacServive.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return julacServive.query(pageParam).$promise
        .then(function (data) {
