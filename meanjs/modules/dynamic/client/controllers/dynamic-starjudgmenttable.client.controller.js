(function () {
  ...
  angular
    .module('organization')
    .controller('StarJudgmentsTableController', StarJudgmentsTableController);

  StarJudgmentsTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'StarJudgmentsServive',
    '$uibModal', 'Upload', 'localStorageService', 'Authentication'];
  function StarJudgmentsTableController($scope, Notification, $log, $window, StarJudgmentsServive, $uibModal, Upload, localStorageService, Authentication) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/dynamic/client/views/starjudgments-modal-form.client.view.html',
        controller: 'StarJudgmentsinfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        StarJudgmentsData: function () {
          //空数据
          return new StarJudgmentsServive();
        },
        //党建类型
        partyid: function () {
          return vm.type;
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/starjudgmentsinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 评星晋级争双强新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 评星晋级争双强删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/starjudgmentsinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 评星晋级争双强修改成功!'});
            })
            .catch(function (err) {
    ...
    function refreshRecordCount(queryParam) {
      StarJudgmentsServive.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return StarJudgmentsServive.query(pageParam).$promise
        .then(function (data) {
