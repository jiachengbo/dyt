(function () {
  ...
  angular
    .module('organization')
    .controller('ThreeServiceTableController', ThreeServiceTableController);

  ThreeServiceTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'ThreeServiceServive', '$uibModal', 'Authentication', 'CommunityService'];
  function ThreeServiceTableController($scope, Notification, $log, $window,
                                       Upload, ThreeServiceServive, $uibModal, Authentication, CommunityService) {
    var vm = this;
    vm.userCommId = '';
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/dynamic/client/views/threeservice-modal-form.client.view.html',
        controller: 'ThreeServiceinfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        ThreeServiceData: function () {
          //空数据
          return new ThreeServiceServive();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/threeserviceinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 三务公开新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 三务公开删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/threeserviceinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 三务公开修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'ThreeServiceTypeTable.name', displayName: '类型'},
    ...
    function refreshRecordCount(queryParam) {
      ThreeServiceServive.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return ThreeServiceServive.query(pageParam).$promise
        .then(function (data) {
