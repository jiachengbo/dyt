(function () {
  ...
  function TopVoiceTableController($scope, Notification, $log, $window,
                                   Upload, TopVoiceService, $uibModal, $stateParams, $state) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          ...
          .then(function (res) {
            ...
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 高层声音新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 高层声音删除成功!'});
        })
          .catch(function (err) {
        ...
        if (isupdate) {
          Upload.upload({
            ...
            .then(function (res) {
              ...
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 高层声音修改成功!'});
            })
            .catch(function (err) {
