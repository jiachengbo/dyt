(function () {
  ...
  angular
    .module('organization')
    .controller('StreetDynamicTableController', StreetDynamicTableController);

  StreetDynamicTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'StreetDynamicsService',
    '$uibModal'];
  function StreetDynamicTableController($scope, Notification, $log, $window,
                                        Upload, StreetDynamicsService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/basicinfo/client/views/streetdynamics-modal-form.client.view.html',
        controller: 'StreetDynamicsinfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        streetdynamicsData: function () {
          //空数据
          return new StreetDynamicsService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/basic/streetdynamicsinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 街道动态新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 街道动态删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/basic/streetdynamicsinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 街道动态修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'content', displayName: '内容', width: 800},
        {field: 'createdate', displayName: '创建时间', cellFilter: 'date:\"yyyy-MM-dd HH:mm:ss\"'},
        {field: 'ispush', displayName: '是否推送首页'}
      ],
    ...
    function refreshRecordCount(queryParam) {
      StreetDynamicsService.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return StreetDynamicsService.query(pageParam).$promise
        .then(function (data) {
