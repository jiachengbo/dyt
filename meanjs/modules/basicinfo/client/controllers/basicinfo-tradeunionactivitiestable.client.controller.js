(function () {
  ...
  angular
    .module('organization')
    .controller('TradeUnionActivitiesTableController', TradeUnionActivitiesTableController);

  TradeUnionActivitiesTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'TradeUnionActivitiesService',
    '$uibModal'];
  function TradeUnionActivitiesTableController($scope, Notification, $log, $window,
                                               Upload, TradeUnionActivitiesService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/basicinfo/client/views/tradeunionactivities-modal-form.client.view.html',
        controller: 'TradeUnionActivitiesinfoModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        tradeunionactivitiesData: function () {
          //空数据
          return new TradeUnionActivitiesService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/basic/tradeunionactivitiesinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 工会活动新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 工会活动删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/basic/tradeunionactivitiesinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 工会活动修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      data: vm.tableData,
      columnDefs: [
        {field: 'name', displayName: '活动名称', width: 350},
        {field: 'type', displayName: '活动类型', width: 100},
        {field: 'content', displayName: '活动内容'},
        {field: 'createdate', displayName: '创建时间', cellFilter: 'date:\"yyyy-MM-dd HH:mm:ss\"', width: 160}
      ],
    ...
    function refreshRecordCount(queryParam) {
      TradeUnionActivitiesService.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return TradeUnionActivitiesService.query(pageParam).$promise
        .then(function (data) {
