(function () {
  ...
  angular
    .module('organization')
    .controller('PioneerExemplaryTableController', PioneerExemplaryTableController);

  PioneerExemplaryTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'PioneerExemplaryService',
    '$uibModal', 'Upload', 'Authentication', 'CommunityService'];
  function PioneerExemplaryTableController($scope, Notification, $log, $window, PioneerExemplaryService, $uibModal, Upload, Authentication, CommunityService) {
    var vm = this;
    vm.userCommId = '';
    //表数据
    vm.tableData = [];
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/basicinfo/client/views/pioneerexemplary-modal-form.client.view.html',
        controller: 'PioneerExemplaryModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        pioneerexemplaryData: function () {
          //空数据
          return new PioneerExemplaryService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        Upload.upload({
          url: '/api/basic/pioneerexemplaryinfo',
          data: result
        })
          .then(function (res) {
            // vm.gridOptions.data.push(res);
            refreshRecordCount(vm.queryParam);
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 先锋模范新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 先锋模范删除成功!'});
        })
          .catch(function (err) {
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/basic/pioneerexemplaryinfo/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
              angular.extend(vm.selectedRow, res.data);
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 先锋模范修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'type', displayName: '事迹类型'},
        {field: 'deeds', displayName: '模范事迹', width: 800},
    ...
    function refreshRecordCount(queryParam) {
      PioneerExemplaryService.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return PioneerExemplaryService.query(pageParam).$promise
        .then(function (data) {
