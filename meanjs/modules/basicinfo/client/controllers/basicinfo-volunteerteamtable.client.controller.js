(function () {
  ...
  angular
    .module('organization')
    .controller('VolunteerTeamTableController', VolunteerTeamTableController);

  VolunteerTeamTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'uiGridConstants', 'VolunteerTeamService',
    '$uibModal'];
  function VolunteerTeamTableController($scope, Notification, $log, $window,
                                        uiGridConstants, VolunteerTeamService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/basicinfo/client/views/volitionteam-modal-form.client.view.html',
        controller: 'VolunteerTeamModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg,
        size: 'lg'
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        ...
        volitionteamData: function () {
          //空数据
          return new VolunteerTeamService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        result.$save()
          .then(function (res) {
            ...
            Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 红雁义工队新增成功!'});
          })
          .catch(function (err) {
    ...
    vm.remove = function () {
      if ($window.confirm('确定要删除吗?')) {
        vm.selectedRow.$remove(function () {
          ...
          //复位当前行
          vm.selectedRow = null;
          Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 红雁义工队删除成功!'});
        })
          .catch(function (err) {
        ...
        if (isupdate) {
          result.$update()
            .then(function (res) {
              ...
              Notification.success({message: '<i class="glyphicon glyphicon-ok"></i> 红雁义工队修改成功!'});
            })
            .catch(function (err) {
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'reasonsforjoining', displayName: '加入原因'},
    ...
    function refreshRecordCount(queryParam) {
      VolunteerTeamService.query(queryParam).$promise
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return VolunteerTeamService.query(pageParam).$promise
        .then(function (data) {
