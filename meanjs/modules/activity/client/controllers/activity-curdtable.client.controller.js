(function () {
  ...
  angular
    .module('activity')
    .controller('ActivityCURDTableController', ActivityCURDTableController);

  ActivityCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'uiGridConstants', 'ActivityService',
    '$uibModal'];
  function ActivityCURDTableController($scope, Notification, $log, $window,
                                       uiGridConstants, ActivityService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/activity/client/views/activity-modal-form.client.view.html',
        controller: 'ActivityModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg
      });
      ...
      modalInstance.result.then(function(result) {
        ...
        result.$save()
          .then(function(res) {
            ...
            Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> activity add saved successfully!' });
          })
          .catch(function(err) {
            $log.error('activity add save error:', err.data.message);
            Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
              ' activity add save error!' });
          });
      })
    ...
    vm.remove = function() {
      if ($window.confirm('Are you sure you want to remove selected record?')) {
        ...
        .catch(function(err) {
          $log.error('activity deleted error:', err.data.message);
          Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
        ...
        if (isupdate) {
          result.$update()
            .then(function(res) {
              ...
              Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> activity update saved successfully!' });
            })
            .catch(function(err) {
              $log.error('activity update save error:', err.data.message);
              Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
              'activity update save error!' });
            });
        }
      }).catch(function(reason) {
        $log.log('Modal dismissed:', reason);
    ...
    vm.gridOptions = {
      ...
      data: vm.tableData,
      columnDefs: [
        {field: 'title', displayName: '活动名称'},
        {field: 'content', displayName: '活动地点'},
        {field: 'User.displayName', displayName: '发布时间'}
      ],
