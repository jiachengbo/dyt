(function () {
  ...
  function PartyMapController($scope, Authentication, $window, partymapService, communityService, partyorgmapService, $state, $rootScope, $timeout) {
    ...
    vm.id = '0';
    // vm.mapdatas = [];
    // var storage = $window.localStorage.getItem('mapOpts');
    // storage = JSON.parse(storage);
    // $scope.mapOpts = {
    //   apiKey: '	Xno2bG5YsPE6EvFfIbKiendQ5jYuTnEA',
    //   center: {longitude: 108.980961, latitude: 34.227858},
    //   zoom: 15,
    ...
    // };
    // $timeout(function () {
    //   // storage = $window.localStorage.getItem('mapOpts');
    //   // storage=JSON.parse(storage);
    //   // $scope.mapOpts.markssss = storage;
    //   // console.log($window.localStorage.getItem('mapOpts'));
    //   $scope.mapOpts.markssss = vm.mapdatas;
    // }, 500);
    ...
    function getmap(id) {
      partymapService.query({comid: id}).$promise.then(function (data) {
        ...
        // var data1 = data[1];
        // for (vm.i = 0; vm.i < 3; vm.i++) {
        //   var mapobj = {};
        //   mapobj.leixin = 0;
        //   mapobj.lat = parseFloat(data1[vm.i].lat);
        //   mapobj.lon = parseFloat(data1[vm.i].lng);
        //   mapobj.resson = data1[vm.i].difficultreason;
        //   mapobj.mand = data1[vm.i].difficultdemand;
        //   mapobj.name = data1[vm.i].name;
        //   mapobj.type = data1[vm.i].persontypename;
        //   mapobj.age = year - parseInt(data1[vm.i].identityid.substring(6, 10));
        //   mapobj.icon = '/modules/core/client/img/image/map/' + data1[vm.i].persontype + '.png';
    ...
    function getorgmap(id) {
      partyorgmapService.query({comid: id}).$promise.then(function (data) {
        ...
        // $window.localStorage.setItem('mapOpts', JSON.stringify(vm.mapdatas));
        // console.log(vm.mapdatas);
        // $scope.mapOpts.markssss = vm.mapdatas;
