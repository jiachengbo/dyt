(function () {
  ...
  angular
    ...
    .controller('PublicprojectCURDTableController', PublicprojectCURDTableController);

  PublicprojectCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'PublicprojectService',
    '$uibModal'];
  function PublicprojectCURDTableController($scope, Notification, $log, $window,
                                            Upload, PublicprojectService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/publicproject/client/views/publicproject-modal-form.client.view.html',
        controller: 'PublicprojectModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg
      });
    ...
    vm.add = function() {
      var modalInstance = vm._openModal({
        //publicproject会传入modal的controller
        publicprojectData: function() {
          //空数据
          return new PublicprojectService();
        },
        //表明是增加
        method: function() {
          return 'add';
      ...
      modalInstance.result.then(function(result) {
        ...
        Upload.upload({
          ...
          .catch(function(err) {
            $log.error('publicproject add save error:', err.data.message);
    ...
    vm.remove = function() {
      if ($window.confirm('确定要删除此条数据?')) {
        ...
        .catch(function(err) {
          $log.error('publicproject deleted error:', err.data.message);
          Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
          ' 删除失败!' });
        });
      }
    ...
    vm._updateorview = function(isupdate) {
      ...
      modalInstance.result.then(function(result) {
        ...
        if (isupdate) {
          Upload.upload({
            url: '/api/publicproject/' + result.id,
            data: result
          })
            .then(function(res) {
              vm.getTableData();
            ...
            .catch(function(err) {
              $log.error('publicproject update save error:', err.data.message);
              Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
              '修改失败!' });
            });
        }
    ...
    //取后台Publicproject表所有数据
    vm.getTableData = function () {
      PublicprojectService.query().$promise.then(function(data) {
