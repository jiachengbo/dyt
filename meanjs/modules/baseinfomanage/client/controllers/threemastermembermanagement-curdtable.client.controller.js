(function () {
  ...
  angular
    ...
    .controller('ThreeMasterMemberManagementCURDTableController', ThreeMasterMemberManagementCURDTableController);

  ThreeMasterMemberManagementCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'ThreeMasterMemberManagementService',
    '$uibModal', 'Authentication', 'CommunityService'];
  function ThreeMasterMemberManagementCURDTableController($scope, Notification, $log, $window, ThreeMasterMemberManagementService, $uibModal, Authentication, CommunityService) {
    var vm = this;
    vm.userCommId = '';
    //表数据
    vm.tableData = [];
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/baseinfomanage/client/views/threemastermembermanagement-modal-form.client.view.html',
        controller: 'ThreeMasterMemberManagementModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        size: 'lg',
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        //threeMasterMemberManagement会传入modal的controller
        threeMasterMemberManagementData: function () {
          //空数据
          return new ThreeMasterMemberManagementService();
        },
        //表明是增加
        method: function () {
          return '新增';
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'birth_date', displayName: '出生日期', cellFilter: 'date:"yyyy-MM-dd"'},
        {field: 'education_degree', displayName: '文化程度'},
        {field: 'work_unit', displayName: '工作单位及职务'},
        {field: 'type_style', displayName: '类型'},
        {field: 'home_address', displayName: '家庭住址'},
        {field: 'contact_information', displayName: '联系方式'},
        {field: 'CommunityVillageConstant.name', displayName: '所属社区'},
        {field: 'grid', displayName: '网格区域', visible: false}
      ],
//-------------分页1.页面操作参数---------------
...
// ---------分页2.分页按钮事件---------------
    ...
    //取后台ThreeMasterMemberManagement表所有数据
    /* ThreeMasterMemberManagementService.query().$promise.then(function(data) {
    ...
    if (Authentication.user !== null) {
      if (Authentication.user.roles[1].indexOf('_') !== -1) {
        ...
        CommunityService.query({
          ...
          vm.queryParam = {
            threeMasterMemberManagementId: 0,
            limit: 0,
            offset: 0,
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return ThreeMasterMemberManagementService.query(pageParam).$promise
        .then(function (data) {
          //序号列的处理
          if (data.length > 0) {
    ...
    function refreshRecordCount(queryParam) {
      ThreeMasterMemberManagementService.query(queryParam).$promise
