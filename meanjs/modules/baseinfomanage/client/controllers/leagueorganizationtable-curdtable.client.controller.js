(function () {
  ...
  angular
    ...
    .controller('LeagueOrganizationTableCURDTableController', LeagueOrganizationTableCURDTableController);

  LeagueOrganizationTableCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'uiGridConstants', 'LeagueOrganizationTableService',
    '$uibModal'];
  function LeagueOrganizationTableCURDTableController($scope, Notification, $log, $window,
                                       uiGridConstants, LeagueOrganizationTableService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/baseinfomanage/client/views/leagueorganizationtable-modal-form.client.view.html',
        controller: 'LeagueOrganizationTableModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        size: 'lg',
    ...
    vm.add = function() {
      var modalInstance = vm._openModal({
        //leagueOrganizationTable会传入modal的controller
        leagueOrganizationTableData: function() {
          //空数据
          return new LeagueOrganizationTableService();
        },
        //表明是增加
        method: function() {
          return '新增';
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'name', displayName: '团组织名称'},
        {field: 'code', displayName: '团组织机构代码'},
        {field: 'organization_type', displayName: '组织类别'},
        {field: 'trade_type', displayName: '行业类别'},
...
//-------------分页1.页面操作参数---------------
    ...
    //取后台LeagueOrganizationTable表所有数据
    /*LeagueOrganizationTableService.query().$promise.then(function(data) {
     vm.gridOptions.data = vm.tableData = data;
     });*/
    //分页3参数
    vm.queryParam = {
      leagueOrganizationTableId: 0,
      limit: 0,
      offset: 0
    };
    //分页4： 刷新记录总数
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return LeagueOrganizationTableService.query(pageParam).$promise
        .then(function (data) {
          //序号列的处理
          if (data.length > 0) {
    ...
    function refreshRecordCount(queryParam) {
      LeagueOrganizationTableService.query(queryParam).$promise
