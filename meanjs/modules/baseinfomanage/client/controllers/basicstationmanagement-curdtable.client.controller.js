(function () {
  ...
  angular
    ...
    .controller('BasicStationManagementCURDTableController', BasicStationManagementCURDTableController);

  BasicStationManagementCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'uiGridConstants', 'BasicStationManagementService',
    '$uibModal'];
  function BasicStationManagementCURDTableController($scope, Notification, $log, $window,
                                       uiGridConstants, BasicStationManagementService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/baseinfomanage/client/views/basicstationmanagement-modal-form.client.view.html',
        controller: 'BasicStationManagementModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        size: 'lg',
    ...
    vm.add = function() {
      var modalInstance = vm._openModal({
        //basicStationManagement会传入modal的controller
        basicStationManagementData: function() {
          //空数据
          return new BasicStationManagementService();
        },
        //表明是增加
        method: function() {
          return '新增';
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'address', displayName: '办公地点'},
...
//-------------分页1.页面操作参数---------------
    ...
    //取后台BasicStationManagement表所有数据
   /* BasicStationManagementService.query().$promise.then(function(data) {
     vm.gridOptions.data = vm.tableData = data;
     });*/
    //分页3参数
    vm.queryParam = {
      basicStationManagementId: 0,
      limit: 0,
      offset: 0
    };
    //分页4： 刷新记录总数
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return BasicStationManagementService.query(pageParam).$promise
        .then(function (data) {
          //序号列的处理
          if (data.length > 0) {
    ...
    function refreshRecordCount(queryParam) {
      BasicStationManagementService.query(queryParam).$promise
