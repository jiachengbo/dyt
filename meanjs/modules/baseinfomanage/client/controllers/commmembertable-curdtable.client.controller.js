(function () {
  ...
  angular
    ...
    .controller('CommMemberTableCURDTableController', CommMemberTableCURDTableController);

  CommMemberTableCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'uiGridConstants', 'CommMemberTableService', '$uibModal', 'Upload'];
  function CommMemberTableCURDTableController($scope, Notification, $log, $window, uiGridConstants, CommMemberTableService, $uibModal, Upload) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/baseinfomanage/client/views/commmembertable-modal-form.client.view.html',
        controller: 'CommMemberTableModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        size: 'lg',
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        //commMemberTable会传入modal的controller
        commMemberTableData: function () {
          //空数据
          return new CommMemberTableService();
        },
        //表明是增加
        method: function () {
          return '新增';
      ...
      modalInstance.result.then(function (result) {
        ...
        // result.$save()
        Upload.upload({
          url: '/api/commMemberTable',
          data: result
        })
          .then(function (res) {
            // vm.tableData.push(new CommMemberTableService(res.data));
//-----------------分页1：新增后， 刷新记录总数---------------
    ...
    vm._updateorview = function (isupdate) {
      ...
      modalInstance.result.then(function (result) {
        ...
        if (isupdate) {
          // result.$update()
          Upload.upload({
            url: '/api/commMemberTable/' + result.id,
            data: result
          })
            .then(function (res) {
              //修改表格显示的数据
...
// ---------分页2.分页按钮事件---------------
    ...
    //取后台CommMemberTable表所有数据
/*    CommMemberTableService.query().$promise.then(function (data) {
...
//分页3参数
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return CommMemberTableService.query(pageParam).$promise
        .then(function (data) {
          //序号列的处理
          if (data.length > 0) {
    ...
    function refreshRecordCount(queryParam) {
      CommMemberTableService.query(queryParam).$promise
