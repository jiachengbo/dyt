(function () {
  ...
  angular
    ...
    .controller('AreadepartmentmanagementCURDTableController', AreadepartmentmanagementCURDTableController);

  AreadepartmentmanagementCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window', 'AreadepartmentmanagementService', '$uibModal', 'Authentication', 'CommunityService', 'localStorageService'];
  function AreadepartmentmanagementCURDTableController($scope, Notification, $log, $window, AreadepartmentmanagementService, $uibModal, Authentication, CommunityService, localStorageService) {
    var vm = this;
    vm.userCommId = '';
    //表数据
    vm.tableData = [];
    ...
    vm._openModal = function (resarg) {
      return $uibModal.open({
        templateUrl: '/modules/baseinfomanage/client/views/areadepartmentmanagement-modal-form.client.view.html',
        controller: 'AreadepartmentmanagementModalFormController',
        controllerAs: 'vm',
        size: 'lg',
        backdrop: 'static',
    ...
    vm.add = function () {
      var modalInstance = vm._openModal({
        //areadepartmentmanagement会传入modal的controller
        areadepartmentmanagementData: function () {
          //空数据
          return new AreadepartmentmanagementService();
        },
        //表明是增加
        method: function () {
          return '新增';
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'leader', displayName: '责任领导'},
        {field: 'chief', displayName: '责任科长'},
        {field: 'fzr', displayName: '责任人'},
...
//-------------分页1.页面操作参数---------------
    ...
    function refreshPageData(pageNumber, pageSize) {
      ...
      //取后台数据，默认按创建时间降序排序
      return AreadepartmentmanagementService.query(pageParam).$promise
        .then(function (data) {
          //序号列的处理
          if (data.length > 0) {
    ...
    function refreshRecordCount(queryParam) {
      AreadepartmentmanagementService.query(queryParam).$promise
    ...
    //取后台Areadepartmentmanagement表所有数据
    /* AreadepartmentmanagementService.query().$promise.then(function (data) {
     vm.gridOptions.data = vm.tableData = data;
     });*/
  }
}());
