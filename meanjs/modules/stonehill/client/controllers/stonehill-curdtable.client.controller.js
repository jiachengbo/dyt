(function () {
  ...
  angular
    .module('stonehill')
    .controller('StonehillCURDTableController', StonehillCURDTableController);

  StonehillCURDTableController.$inject = ['$scope', 'Notification', '$log', '$window',
    'Upload', 'StonehillService',
    '$uibModal'];
  function StonehillCURDTableController($scope, Notification, $log, $window,
                                        Upload, StonehillService, $uibModal) {
    var vm = this;
    //表数据
    vm.tableData = [];
    //ui-grid 当前选择的行
    ...
    vm._openModal = function(resarg) {
      return $uibModal.open({
        templateUrl: '/modules/stonehill/client/views/stonehill-modal-form.client.view.html',
        controller: 'StonehillModalFormController',
        controllerAs: 'vm',
        backdrop: 'static',
        resolve: resarg
      });
    ...
    vm.add = function() {
      var modalInstance = vm._openModal({
        //stonehill会传入modal的controller
        stonehillData: function() {
          //空数据
          return new StonehillService();
        },
        //表明是增加
        method: function() {
          return 'add';
      ...
      modalInstance.result.then(function(result) {
        ...
        Upload.upload({
          ...
          .catch(function(err) {
            $log.error('stonehill add save error:', err.data.message);
    ...
    vm.remove = function() {
      if ($window.confirm('确定要删除此条数据吗?')) {
        ...
        .catch(function(err) {
          $log.error('stonehill deleted error:', err.data.message);
          Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i>' +
          ' 删除失败!' });
        });
      }
      ...
      modalInstance.result.then(function(result) {
        ...
        if (isupdate) {
          Upload.upload({
            ...
            .catch(function(err) {
              $log.error('stonehill update save error:', err.data.message);
              Notification.error({ message: err.data.message, title: '<i class="glyphicon glyphicon-remove"></i> ' +
              '更新失败!' });
            });
        }
    ...
    vm.gridOptions = {
      ...
      columnDefs: [
        ...
        {field: 'time', displayName: '时间', cellFilter: 'date: \"yyyy-MM-dd HH:mm:ss\"'}
      ],
    ...
    vm.getTableData = function () {
      StonehillService.query().$promise.then(function(data) {
